...
public class LoginController {
   ...
   /* @RequestMapping(value="/login",method=RequestMethod.POST)
    public String login(String username, String passowrd, RedirectAttributes redirectAttributes){

        UsernamePasswordToken token = new UsernamePasswordToken(username,passowrd);
        //UsernamePasswordToken token = new UsernamePasswordToken(username, CommonUtils.encrypt(passowrd));
        //获取当前的Subject
...
*/
    @RequestMapping(value="/login",method=RequestMethod.POST)
    public String login(SysUser user, boolean rememberMe, String gifCode,
        ...
        /*if(currentUser.isAuthenticated()){
            ...
            request.getSession().setAttribute("currentUser",sysUser);
            request.getSession().setAttribute("loginName",user.getLoginName());
            logger.info("@@@@@@@@@@@@@session："+request.getSession().getAttribute("loginName"));
            return "redirect:index";
        }else{
            token.clear();
            return "redirect:login";
