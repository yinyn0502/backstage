	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			// 用户-菜单
			var zNodes=[
					<#list menuList as menu>{id:"${menu.id?c}", pId:"<#if menu.parentId??>${menu.parentId?c}<#else>0</#if>", name:"<#if menu.name??>${menu.name}<#else>'菜单'</#if>"},
		            </#list>];
			// 初始化树结构
            ...
            var ids = "<#if cate??>${cate}<#else></#if>".split(",");
