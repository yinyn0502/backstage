...
<#if isCanReverse??&&isCanReverse>
    <script type="text/javascript">
        ...
        $(function() {
            json =<#if jsonString??>${jsonString};</#if>

            var contentStr = "<option>请选择</option>";
            $.each(json, function(index, row) {
                contentStr+="<option value=" + row.code + ">" + row.name + "</option>";

            });
            $("#province").html(contentStr);
...
<#if isCanReverse??&&isCanReverse>
<form id="inputForms" modelAttribute="reverseOrderSaveTO" action="/order/return/apply" method="post" class="form-horizontal">
<#--<input type="hidden" name="reverseId" <#if reverseOrderDetail.reverseId??>value="${reverseOrderDetail.reverseId?c}"</#if>/>-->
    ...
    <table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
        ...
        <tr>
            ...
            <td class="width-35"  >
                <select id="provinceCity"  class="form-control" name="reverseOrderTO.refundReason">
                    <!--<option>请选择</option>-->
                </select>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="width-35" >
                <textarea name="reasonDesc" class="form-control"></textarea>
            </td>
        </tr>
    ...
    <div id="reverseAmount" class="refundView">
        <table>
            ...
            </tr><tr><td style="color: #6f3cc4">预计退款</td></tr>
            <tr id="refundType">
...
<div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary" id="btn_confirm" onclick="gensubmit()">申请退换货</button>
    <script type="text/javascript">
        function gensubmit(){
            doSubmit();
        }
...
<script type="text/javascript">
    function praise() {
        ...
        var requestParams = $("#inputForms").serialize();
        $.ajax({
