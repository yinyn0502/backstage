...
<form id="inputForm" modelAttribute="area" action="/sys/area/save" method="post" class="form-horizontal">
        ...
        <tr>
            ...
            <td class="width-35" >
               <#-- <sys:treeselect id="area" name="parent.id" value="${area.parent.id}" labelName="parent.name" labelValue="${area.parent.name}"
                  ...
                  <input id="parentId" name="parentId" class="form-control m-s" type="hidden" <#if area.parentId??>value="${area.parentId}"</#if>/>
                   <input id="parentIds" name="parentIds" class="form-control m-s" type="hidden" <#if area.parentIds??>value="${area.parentIds}" </#if>/>
                   <input id="sort" name="sort" class="form-control m-s" type="hidden" <#if area.sort??>value="${area.sort}"</#if>/>
                   <input id="delFlag" name="delFlag" class="form-control m-s" type="hidden" <#if area.delFlag??>value="${area.delFlag}"</#if>/>

                   <div class="input-group">
                       <#if area.parent??>
                       <input id="areaName" name="parent.name" readonly="readonly"  type="text" <#if area.parent.name??>value="${area.parent.name}"</#if> data-msg-required=""
